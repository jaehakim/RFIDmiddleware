...
public class DashboardHtml {
    ...
    private static String getStyles() {
        return "*{margin:0;padding:0;box-sizing:border-box}\n"
            ...
            + ".modal .modal-btns button:disabled{background:#aaa!important;color:#fff!important;cursor:not-allowed}\n";
    }
    ...
    private static String getBodyHtml() {
        return "<header>\n"
            ...
            + "  <button onclick=\"switchTab('finder')\">리더기 검색</button>\n"
            + "  <button onclick=\"switchTab('tags')\">태그 데이터</button>\n"
            + "  <button onclick=\"switchTab('assets')\">자산 목록</button>\n"
            + "  <button onclick=\"switchTab('permissions')\">반출허용 목록</button>\n"
            + "  <button onclick=\"switchTab('alerts')\">반출알림 이력</button>\n"
            + "</div>\n"
            + "\n"
            // Readers tab
            + "<div id=\"tab-readers\" class=\"tab-content active\">\n"
            + "  <h3 style=\"margin-bottom:12px;color:#1B2A4A\">[ \ub9ac\ub354\uae30 \uc0c1\ud0dc ]</h3>\n"
            ...
            + "  <h3 style=\"margin-bottom:12px;color:#1B2A4A\">[ \ub9ac\ub354\uae30 \uac80\uc0c9 ]</h3>\n"
            + "  <div class=\"toolbar\">\n"
            + "    <button onclick=\"discoverReaders()\" id=\"btn-discover\">\ub124\ud2b8\uc6cc\ud06c \uac80\uc0c9</button>\n"
            + "    <span id=\"finder-status\" style=\"font-size:13px;color:#666\"></span>\n"
            + "  </div>\n"
            + "  <div id=\"finder-table\"></div>\n"
            + "</div>\n"
            + "\n"
            // Reader network config modal
            + "<div id=\"finder-modal\" class=\"modal-overlay\">\n"
            + "  <div class=\"modal\">\n"
            + "    <h3>\ub9ac\ub354\uae30 \ub124\ud2b8\uc6cc\ud06c \uc124\uc815</h3>\n"
            + "    <div class=\"field\"><label>MAC \uc8fc\uc18c</label><input id=\"fm-mac\" readonly></div>\n"
            + "    <div class=\"field\"><label>IP \ud560\ub2f9</label><select id=\"fm-dhcp\" onchange=\"toggleDhcp()\"><option value=\"1\">DHCP (\uc790\ub3d9)</option><option value=\"0\">\uace0\uc815 IP (\uc218\ub3d9)</option></select></div>\n"
            + "    <div class=\"field\"><label>IP \uc8fc\uc18c</label><input id=\"fm-ip\" placeholder=\"192.168.0.100\"></div>\n"
            + "    <div class=\"field\"><label>\uc11c\ube0c\ub137 \ub9c8\uc2a4\ud06c</label><input id=\"fm-subnet\" placeholder=\"255.255.255.0\"></div>\n"
            + "    <div class=\"field\"><label>\uac8c\uc774\ud2b8\uc6e8\uc774</label><input id=\"fm-gateway\" placeholder=\"192.168.0.1\"></div>\n"
            + "    <div class=\"modal-btns\">\n"
            + "      <button class=\"btn-cancel\" onclick=\"closeFinderModal()\">취소</button>\n"
            + "      <button class=\"btn-save\" onclick=\"applyFinderConfig()\">적용</button>\n"
            + "      <button style=\"background:#e67e22;color:#fff\" onclick=\"restartFinderReader()\">재시작</button>\n"
            + "    </div>\n"
            + "  </div>\n"
            + "</div>\n"
            + "\n"
            ...
            + "  <h3 style=\"margin-bottom:12px;color:#1B2A4A\">[ \ubc18\ucd9c\ud5c8\uc6a9 \ubaa9\ub85d ]</h3>\n"
            ...
            + "  <h3 style=\"margin-bottom:12px;color:#1B2A4A\">[ \ubc18\ucd9c\uc54c\ub9bc \uc774\ub825 ]</h3>\n"
            + "  <div class=\"toolbar\">\n"
            ...
            + "    <div class=\"field\"><label>IP \uc8fc\uc18c</label><input id=\"rm-ip\"></div>\n"
            + "    <div class=\"field\"><label>\ud3ec\ud2b8</label><input id=\"rm-port\" type=\"number\"></div>\n"
            + "    <div class=\"field\"><label>\ube44\ud504\uc74c</label><select id=\"rm-beep\"><option value=\"true\">\uc801\uc6a9</option><option value=\"false\">\ud574\uc81c</option></select></div>\n"
    ...
    private static String getScript() {
        return "function switchTab(name){\n"
            ...
            + "  var tabs=['readers','finder','tags','assets','permissions','alerts'];\n"
            ...
            + "    var h='<table><tr><th>\uc774\ub984</th><th>IP \uc8fc\uc18c</th><th>\ud3ec\ud2b8</th><th>\ube44\ud504\uc74c</th><th>\uacbd\uad11\ub4f1</th><th>\ubd80\uc800</th><th>\uc548\ud14c\ub098 \ucd9c\ub825</th><th>\ub4dc\uc6f0\uc2dc\uac04</th><th>\uc0c1\ud0dc</th></tr>';\n"
            ...
            + "  document.getElementById('rm-beep').value=String(x.beepEnabled);\n"
            ...
            + "    beepEnabled:document.getElementById('rm-beep').value==='true',\n"
            ...
            + "    var h='<table><tr><th>\uc790\uc0b0\ubc88\ud638</th><th>EPC</th><th>\uc790\uc0b0\uba85</th><th>\ubd80\uc11c</th><th>\ub4f1\ub85d\uc77c\uc2dc</th><th>\ubcf4\uc720\uc5ec\ubd80</th></tr>';\n"
            + "    for(var i=0;i<d.length;i++){\n"
            + "      h+='<tr><td>'+esc(d[i].assetNumber)+'</td><td>'+esc(d[i].epc)+'</td><td>'+esc(d[i].assetName)+'</td><td>'+esc(d[i].department)+'</td><td>'+esc(d[i].createdAt)+'</td><td>'+(d[i].possession?'\\ubcf4\\uc720':'\\ubbf8\\ubcf4\\uc720')+'</td></tr>';\n"
            + "    }\n"
            + "    h+='</table>';\n"
            ...
            + "    var h='<table><tr><th>\uc54c\ub9bc\uc2dc\uac04</th><th>\ub9ac\ub354\uae30</th><th>EPC</th><th>\uc790\uc0b0\ubc88\ud638</th><th>\uc790\uc0b0\uba85</th><th>\ubd80\uc11c</th><th>RSSI</th></tr>';\n"
            + "    for(var i=0;i<d.length;i++){\n"
            + "      h+='<tr><td>'+esc(d[i].alertTime)+'</td><td>'+esc(d[i].readerName)+'</td><td>'+esc(d[i].epc)+'</td><td>'+esc(d[i].assetNumber)+'</td><td>'+esc(d[i].assetName)+'</td><td>'+esc(d[i].department)+'</td><td>'+d[i].rssi+'</td></tr>';\n"
            + "    }\n"
            + "    h+='</table>';\n"
            ...
            + "    var h='<table><tr><th>\uc2dc\uac04</th><th>\ub9ac\ub354\uae30</th><th>EPC</th><th>RSSI</th><th>\uc548\ud14c\ub098</th><th>\uc790\uc0b0\ubc88\ud638</th><th>\uc790\uc0b0\uba85</th><th>\ubd80\uc11c</th><th>\uc0c1\ud0dc</th></tr>';\n"
            ...
            + "      h+='<td>'+esc(t.epc)+'</td>';\n"
            + "      h+='<td>'+t.rssi+'</td>';\n"
            + "      h+='<td>'+(t.antenna!=null?t.antenna:'')+'</td>';\n"
            ...
            + "  document.getElementById('finder-status').textContent='\ub124\ud2b8\uc6cc\ud06c\uc5d0\uc11c \ub9ac\ub354\uae30\ub97c \uac80\uc0c9\ud558\uace0 \uc788\uc2b5\ub2c8\ub2e4 (\uc57d 3\ucd08)...';\n"
            + "  document.getElementById('finder-table').innerHTML='<div class=\"empty-msg\">\uac80\uc0c9 \uc911...</div>';\n"
            + "  api('POST','/api/reader-finder/discover').then(function(r){\n"
            + "    btn.disabled=false;btn.textContent='\ub124\ud2b8\uc6cc\ud06c \uac80\uc0c9';\n"
            + "    if(r.status!=='ok'){document.getElementById('finder-table').innerHTML='<div class=\"empty-msg\">'+r.message+'</div>';document.getElementById('finder-status').textContent='';return;}\n"
            + "    _finderDevices=r.data;\n"
            + "    document.getElementById('finder-status').textContent='\uac80\uc0c9 \uacb0\uacfc: '+_finderDevices.length+'\ub300';\n"
            + "    if(!_finderDevices.length){document.getElementById('finder-table').innerHTML='<div class=\"empty-msg\">\ubc1c\uacac\ub41c \ub9ac\ub354\uae30\uac00 \uc5c6\uc2b5\ub2c8\ub2e4</div>';return;}\n"
            + "    var h='<table><tr><th>MAC \uc8fc\uc18c</th><th>IP \uc8fc\uc18c</th><th>IP \ud560\ub2f9</th><th>\ud3ec\ud2b8</th><th>\uc11c\ube0c\ub137</th><th>\uac8c\uc774\ud2b8\uc6e8\uc774</th><th>\ubaa8\ub378</th><th>\ud38c\uc6e8\uc5b4</th><th></th></tr>';\n"
            + "    for(var i=0;i<_finderDevices.length;i++){\n"
            + "      var d=_finderDevices[i];\n"
            + "      var dhcpText=d.dhcp==='1'?'DHCP':d.dhcp==='0'?'\uace0\uc815 IP':'';\n"
            + "      h+='<tr>';\n"
            + "      h+='<td>'+esc(d.mac_address||d.mac||'')+'</td>';\n"
            + "      h+='<td>'+esc(d.ip||d.ip_address||'')+'</td>';\n"
            + "      h+='<td>'+esc(dhcpText)+'</td>';\n"
            + "      h+='<td>'+esc(d.port||'')+'</td>';\n"
            + "      h+='<td>'+esc(d.subnet||d.mask||'')+'</td>';\n"
            + "      h+='<td>'+esc(d.gateway||'')+'</td>';\n"
            + "      h+='<td>'+esc(d.name||d.model||'')+'</td>';\n"
            + "      h+='<td>'+esc(d.version||'')+'</td>';\n"
            + "      h+='<td><button onclick=\"openFinderConfig('+i+')\" style=\"padding:3px 10px;font-size:12px\">\uc124\uc815</button></td>';\n"
            + "      h+='</tr>';\n"
            + "    }\n"
            + "    h+='</table>';\n"
            + "    document.getElementById('finder-table').innerHTML=h;\n"
            + "  }).catch(function(e){\n"
            + "    btn.disabled=false;btn.textContent='\ub124\ud2b8\uc6cc\ud06c \uac80\uc0c9';\n"
            + "    document.getElementById('finder-status').textContent='';\n"
            + "    document.getElementById('finder-table').innerHTML='<div class=\"empty-msg\">\uc624\ub958: '+e.message+'</div>';\n"
            + "  });\n"
            + "}\n"
            + "\n"
            + "function toggleDhcp(){\n"
            + "  var isDhcp=document.getElementById('fm-dhcp').value==='1';\n"
            + "  document.getElementById('fm-ip').disabled=isDhcp;\n"
            + "  document.getElementById('fm-subnet').disabled=isDhcp;\n"
            + "  document.getElementById('fm-gateway').disabled=isDhcp;\n"
            + "}\n"
            + "\n"
            + "function openFinderConfig(idx){\n"
            + "  var d=_finderDevices[idx];\n"
            + "  var mac=d.mac_address||d.mac||'';\n"
            + "  document.getElementById('fm-mac').value=mac;\n"
            + "  document.getElementById('fm-ip').value=d.ip||d.ip_address||'';\n"
            + "  document.getElementById('fm-subnet').value=d.subnet||d.mask||d.subnet_mask||'255.255.255.0';\n"
            + "  document.getElementById('fm-gateway').value=d.gateway||d.default_gateway||'';\n"
            + "  document.getElementById('fm-dhcp').value=d.dhcp||'1';\n"
            + "  toggleDhcp();\n"
            + "  document.getElementById('finder-modal').classList.add('show');\n"
            + "}\n"
            + "\n"
            + "function closeFinderModal(){document.getElementById('finder-modal').classList.remove('show');}\n"
            + "\n"
            + "function applyFinderConfig(){\n"
            + "  var mac=document.getElementById('fm-mac').value;\n"
            + "  if(!mac){showToast('\ub9ac\ub354\uae30\ub97c \uba3c\uc800 \uc120\ud0dd\ud558\uc138\uc694','error');return;}\n"
            + "  var body={\n"
            + "    dhcp:document.getElementById('fm-dhcp').value,\n"
            + "    ip:document.getElementById('fm-ip').value,\n"
            + "    subnet:document.getElementById('fm-subnet').value,\n"
            + "    gateway:document.getElementById('fm-gateway').value\n"
            + "  };\n"
            + "  if(body.dhcp==='0'&&!body.ip){showToast('IP \uc8fc\uc18c\ub97c \uc785\ub825\ud558\uc138\uc694','error');return;}\n"
            + "  var applyBtn=document.querySelector('#finder-modal .btn-save');\n"
            + "  var restartBtn=document.querySelector('#finder-modal button[onclick*=restartFinderReader]');\n"
            + "  var cancelBtn=document.querySelector('#finder-modal .btn-cancel');\n"
            + "  applyBtn.disabled=true;applyBtn.textContent='\uc124\uc815 \ubcc0\uacbd \uc911...';\n"
            + "  restartBtn.disabled=true;cancelBtn.disabled=true;\n"
            + "  document.getElementById('finder-status').textContent='\uc124\uc815 \uc801\uc6a9 \uc911... (\uc57d 10\ucd08 \uc18c\uc694)';\n"
            + "  api('PUT','/api/reader-finder/config/'+encodeURIComponent(mac),body).then(function(r){\n"
            + "    applyBtn.disabled=false;applyBtn.textContent='\uc801\uc6a9';\n"
            + "    restartBtn.disabled=false;cancelBtn.disabled=false;\n"
            + "    if(r.status==='ok'){\n"
            + "      document.getElementById('finder-status').textContent='\u2705 \uc124\uc815 \uc801\uc6a9 \uc644\ub8cc';\n"
            + "      showToast('\ub124\ud2b8\uc6cc\ud06c \uc815\ubcf4\uac00 \ubcc0\uacbd\ub418\uc5c8\uc2b5\ub2c8\ub2e4. \\n\\'\\ub124\\ud2b8\\uc6cc\\ud06c \\uac80\\uc0c9\\'\\uc744 \\ub2e4\\uc2dc \\uc9c4\\ud589\\ud574 \\uc8fc\\uc2ed\\uc2dc\\uc624.','success');\n"
            + "      closeFinderModal();\n"
            + "    } else {\n"
            + "      document.getElementById('finder-status').textContent='\u274c '+r.message;\n"
            + "      showToast(r.message,'error');\n"
            + "    }\n"
            + "  }).catch(function(e){\n"
            + "    applyBtn.disabled=false;applyBtn.textContent='\uc801\uc6a9';\n"
            + "    restartBtn.disabled=false;cancelBtn.disabled=false;\n"
            + "    document.getElementById('finder-status').textContent='\u274c \uc624\ub958: '+e.message;\n"
            + "    showToast('\uc624\ub958: '+e.message,'error');\n"
            + "  });\n"
            + "}\n"
            + "\n"
            + "function restartFinderReader(){\n"
            + "  var mac=document.getElementById('fm-mac').value;\n"
            + "  if(!mac){showToast('\ub9ac\ub354\uae30\ub97c \uba3c\uc800 \uc120\ud0dd\ud558\uc138\uc694','error');return;}\n"
            + "  if(!confirm('MAC: '+mac+'\\n\ub9ac\ub354\uae30\ub97c \uc7ac\uc2dc\uc791\ud558\uc2dc\uaca0\uc2b5\ub2c8\uae4c?'))return;\n"
            + "  var applyBtn=document.querySelector('#finder-modal .btn-save');\n"
            + "  var restartBtn=document.querySelector('#finder-modal button[onclick*=restartFinderReader]');\n"
            + "  var cancelBtn=document.querySelector('#finder-modal .btn-cancel');\n"
            + "  restartBtn.disabled=true;restartBtn.textContent='\uc7ac\uc2dc\uc791 \uc911...';\n"
            + "  applyBtn.disabled=true;cancelBtn.disabled=true;\n"
            + "  document.getElementById('finder-status').textContent='\ub9ac\ub354\uae30 \uc7ac\uc2dc\uc791 \uc911...';\n"
            + "  api('POST','/api/reader-finder/restart/'+encodeURIComponent(mac)).then(function(r){\n"
            + "    restartBtn.disabled=false;restartBtn.textContent='\uc7ac\uc2dc\uc791';\n"
            + "    applyBtn.disabled=false;cancelBtn.disabled=false;\n"
            + "    if(r.status==='ok'){\n"
            + "      document.getElementById('finder-status').textContent='\u2705 \ub9ac\ub354\uae30 \uc7ac\uc2dc\uc791 \uc694\uccad \uc644\ub8cc';\n"
            + "      showToast('\ub9ac\ub354\uae30 \uc7ac\uc2dc\uc791 \uc694\uccad \uc644\ub8cc','success');\n"
            + "      closeFinderModal();\n"
            + "    } else {\n"
            + "      document.getElementById('finder-status').textContent='\u274c '+r.message;\n"
            + "      showToast(r.message,'error');\n"
